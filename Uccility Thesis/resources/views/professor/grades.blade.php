	...
	<div class="panel-body student-records">
		<table id="grading-table" class="table table-bordered">
			<tr>
				...
				@if($criterias[0]->term == 'Midterm')
					<th colspan="14" style="text-align: center;">{{ $criterias[0]->term }} 25%</th>
				@elseif($criterias[0]->term == 'Finalterm')
					<th colspan="14" style="text-align: center;">{{ $criterias[0]->term }} 75%</th>
				@else
					<th>Unknown</th>
				@endif
			</tr>
						...
						@foreach($criterias as $criteria)
							@if(count($criteria->getCriteriaItems) > 0)
								@foreach($criteria->getCriteriaItems as $key => $item)
									...
									@else
										<td class="{{ $criteria->criteria.$keys.$key }}" user-id="{{ $student->user_id }}" criteria-item="{{ $item->id }}" contenteditable="true" total="{{ $item->total }}">0</td>
									@endif
								@endforeach
							@else
								<td>100</td>
